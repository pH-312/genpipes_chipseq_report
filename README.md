# README for `genpipes_chipseq_report`
...
To understand differences between `genpipes_chipseq_report` & `genpipes_chipseq_report_with_wrapper`, see section below.

## Set-up
1. Create directory `ReportingFigures` within projects.
2. Set up `ReportingFigures` directory as follows:
```bash
ReportingFigures
  ├── README.md
...
### Requirements
**For ChIP-seq:** 
Must run Genpipes (version 6) steps 1 (if necessary), 2-18 and 20-21.

**For ATAC-seq:** 
Must run Genpipes (version 6) steps 1 (if necessary), 2-17 and 19-20 (with "-t atacseq" option).\
