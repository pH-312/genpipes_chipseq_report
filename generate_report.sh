...
###################################################################################################################
# Version 6.0: 2025-08-19 
# This version does not implement the wrapper and only requires 2/3 arguments to be specified
...
#           HOW TO USE            #
###################################
# From the report directory of Genpipes' output, submit generate_report.sh to the SLURM, specifying 2 mandatory arguments:
# ARG1: Project ID (mandatory)
# ARG2: Primary contact (mandatory)
# ARG3: chipseq or atacseq (optional, defaults to chipseq) #TMP
  # TMP: must specify "atacseq" as 3rd arg for ATAC-seq data
  # (Script finds seq_type in most recent ini so running deepTools with "-t chipseq" makes seq_type "chipseq", see below)
  # (Once deepTools is implemented, no more need to specify "atacseq", this 3rd argument can be removed)

# EXAMPLE: [user@narval report]$ sbatch path_to/generate_report.sh "Project ID" "Primary Contact" "atacseq"

# generate_report.sh does the following:
...
if [ "$(basename "$(pwd)")" != "report" ]; then
    ...
    echo "Please run from the report directory of GenPipes' output." 
    echo "Exiting."
    exit 1
fi
...
if [ -z "$seq_type" ]; then # seq_type argument not specified, default to chipseq
  seq_type="chipseq"
elif [[ "$seq_type" != "chipseq" && "$seq_type" != "atacseq" ]]; then # Check validity of seq_type argument
  echo "Invalid input for sequencing type. Use \"chipseq\" or \"atacseq\". Exiting."
  exit 1
fi
# For when ATAC-seq pipeline is updated with deepTools:
...
# - Readset name --------------------------------------------------------
## Get readset name from ini command:
...
echo "Project ID (arg1):      $project_id"
echo "Primary contact (arg2): $primary_contact" $'\n'
echo "Sequencing data type:   $seq_type"
echo "Ini file:               $(basename "$ini_file")"
echo "Readset file name:      $readset_name" $'\n'
...
if [[ "$seq_type" == "chipseq" ]]; then # For ChIP-seq: Not mandatory - no HOMER motif analysis if no narrow peak marks are used
...
# Create PCA plot PDF
plotPCA -in "$read_coverage" -o "$graphs_dir/read_coverage_pca.pdf" -l "${label_list[@]}" --markers "${marker_list[@]}" -T "Read coverage PCA"\
...
report_html=$(find -maxdepth 1 -type f -name "${rmd_name%.Rmd}.html") \
  && { echo $'\n'"Report HTML created at: $report_html" $'\n'; } \
